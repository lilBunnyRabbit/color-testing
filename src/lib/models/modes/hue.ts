...
function rgbToHsp(R: number, G: number, B: number): Vec {
	...
	let H = 0, S = 0;
	if (R === G && G === B) { H = 0; S = 0; }
	else if (R >= G && R >= B) { if (B >= G) { H = 1 - (1 / 6) * (B - G) / (R - G); S = 1 - G / R; } else { H = (1 / 6) * (G - B) / (R - B); S = 1 - B / R; } }
	else if (G >= R && G >= B) { if (R >= B) { H = 2 / 6 - (1 / 6) * (R - B) / (G - B); S = 1 - B / G; } else { H = 2 / 6 + (1 / 6) * (B - R) / (G - R); S = 1 - R / G; } }
	else { if (G >= R) { H = 4 / 6 - (1 / 6) * (G - R) / (B - R); S = 1 - R / B; } else { H = 4 / 6 + (1 / 6) * (R - G) / (B - G); S = 1 - G / B; } }
	return [H * 360, S, P];
}
function hspToRgb(Hd: number, S: number, P: number): Vec {
	let H = (((Hd % 360) + 360) % 360) / 360;
	let R = 0, G = 0, B = 0, part: number;
	const m = 1 - S;
	if (m > 0) {
		if (H < 1 / 6) { H = 6 * H; part = 1 + H * (1 / m - 1); B = P / sqf(Pr / m / m + Pg * part * part + Pb); R = B / m; G = B + H * (R - B); }
		else if (H < 2 / 6) { H = 6 * (-H + 2 / 6); part = 1 + H * (1 / m - 1); B = P / sqf(Pg / m / m + Pr * part * part + Pb); G = B / m; R = B + H * (G - B); }
		else if (H < 3 / 6) { H = 6 * (H - 2 / 6); part = 1 + H * (1 / m - 1); R = P / sqf(Pg / m / m + Pb * part * part + Pr); G = R / m; B = R + H * (G - R); }
		else if (H < 4 / 6) { H = 6 * (-H + 4 / 6); part = 1 + H * (1 / m - 1); R = P / sqf(Pb / m / m + Pg * part * part + Pr); B = R / m; G = R + H * (B - R); }
		else if (H < 5 / 6) { H = 6 * (H - 4 / 6); part = 1 + H * (1 / m - 1); G = P / sqf(Pb / m / m + Pr * part * part + Pg); B = G / m; R = G + H * (B - G); }
		else { H = 6 * (-H + 1); part = 1 + H * (1 / m - 1); G = P / sqf(Pr / m / m + Pb * part * part + Pg); R = G / m; B = G + H * (R - G); }
	} else {
		if (H < 1 / 6) { H = 6 * H; R = sqf((P * P) / (Pr + Pg * H * H)); G = R * H; B = 0; }
		else if (H < 2 / 6) { H = 6 * (-H + 2 / 6); G = sqf((P * P) / (Pg + Pr * H * H)); R = G * H; B = 0; }
		else if (H < 3 / 6) { H = 6 * (H - 2 / 6); G = sqf((P * P) / (Pg + Pb * H * H)); B = G * H; R = 0; }
		else if (H < 4 / 6) { H = 6 * (-H + 4 / 6); B = sqf((P * P) / (Pb + Pg * H * H)); G = B * H; R = 0; }
		else if (H < 5 / 6) { H = 6 * (H - 4 / 6); B = sqf((P * P) / (Pb + Pr * H * H)); R = B * H; G = 0; }
		else { H = 6 * (-H + 1); R = sqf((P * P) / (Pr + Pb * H * H)); B = R * H; G = 0; }
	}
	return [R, G, B];
}
defMode('hsp', ['h', 's', 'p'], 'rgb',
...
function rgbToTsl(R: number, G: number, B: number): Vec {
	const sum = R + G + B;
	const r = sum === 0 ? 1 / 3 : R / sum, g = sum === 0 ? 1 / 3 : G / sum;
	const rp = r - 1 / 3, gp = g - 1 / 3;
...
function tslToRgb(T: number, S: number, L: number): Vec {
	...
	if (S <= 1e-12) { rp = 0; gp = 0; }
	else {
		const ang = (T - 1 / 4) * 2 * Math.PI;
		if (Math.abs(Math.cos(ang)) < 1e-9) { gp = 0; rp = (Math.sin(ang) > 0 ? 1 : -1) * sqf((5 * S * S) / 9); }
		else { const x = Math.tan(ang); gp = (T > 0.5 ? -1 : 1) * sqf((5 * S * S) / (9 * (x * x + 1))); rp = x * gp; }
	}
	const r = rp + 1 / 3, g = gp + 1 / 3, b = 1 - r - g;
	const k = 0.299 * r + 0.587 * g + 0.114 * b;
	const total = k === 0 ? 0 : L / k;
	return [r * total, g * total, b * total];
}
defMode('tsl', ['t', 's', 'l'], 'rgb',
...
const RYB_CUBE = [
	[1, 1, 1], [1, 0, 0], [1, 1, 0], [0.163, 0.373, 0.6],
	[1, 0.5, 0], [0.5, 0, 0.5], [0, 0.66, 0.2], [0.2, 0.094, 0.0]
];
export function rybToRgb(r: number, y: number, b: number): Vec {
	...
	for (let k = 0; k < 3; k++) {
		...
		const y0 = x0 * (1 - y) + x1 * y, y1 = x2 * (1 - y) + x3 * y;
		out[k] = y0 * (1 - b) + y1 * b;
	}
	return out;
}
function rgbToRyb(R: number, G: number, B: number): Vec {
	...
	for (let it = 0; it < 40; it++) {
		...
		v = [Math.max(0, Math.min(1, v[0] - d[0])), Math.max(0, Math.min(1, v[1] - d[1])), Math.max(0, Math.min(1, v[2] - d[2]))];
		if (Math.abs(d[0]) + Math.abs(d[1]) + Math.abs(d[2]) < 1e-9) break;
	}
	return v;
}
defMode('ryb', ['r', 'y', 'b'], 'rgb',
