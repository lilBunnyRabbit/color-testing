...
register(
	defineModel({
		...
		ownMethods: [
			method('scaleChroma', [p('factor')], 'color', 'Scale the I/Q chroma channels (NTSC saturation)', (self, [f]) => {
				const c = self.project('yiq') as unknown as Record<string, number | undefined>;
				const k = num(f);
				return ColorValue.from({ mode: 'yiq', y: c.y ?? 0, i: (c.i ?? 0) * k, q: (c.q ?? 0) * k } as unknown as CuloriColor);
			})
		],
		inherit: LAB_OPS
	})
