...
import { method, num, color, wrapHue, p } from '../util';
...
register(
	defineModel({
		...
		channels: [
			...
			{ key: 'hsp_p', localKey: 'p', label: 'Perceived brightness', culoriField: 'p', range: [0, 1] }
		],
		ownMethods: [
			method('rotateHue', [p('degrees')], 'color', 'Rotate hue at constant perceived brightness', (self, [d]) => {
				const c = self.project('hsp') as unknown as Record<string, number | undefined>;
				return ColorValue.from({ mode: 'hsp', h: wrapHue((c.h ?? 0) + num(d)), s: c.s ?? 0, p: c.p ?? 0 } as unknown as CuloriColor);
			})
		]
	})
);
...
register(
	defineModel({
		...
		ownMethods: [
			method('rotateTint', [p('degrees')], 'color', 'Rotate the tint angle (hue analogue)', (self, [d]) => {
				const c = self.project('tsl') as unknown as Record<string, number | undefined>;
				let t = (c.t ?? 0) + num(d) / 360;
				t = ((t % 1) + 1) % 1;
				return ColorValue.from({ mode: 'tsl', t, s: c.s ?? 0, l: c.l ?? 0 } as unknown as CuloriColor);
			})
		]
	})
);
		...
		ownMethods: [
			method(
				...
				(self, [o, t]) => {
					...
					return ColorValue.from({ mode: 'ryb', r: lerp('r'), y: lerp('y'), b: lerp('b') } as unknown as CuloriColor);
				}
			)
		]
	})
