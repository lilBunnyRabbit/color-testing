<script lang="ts">
	...
	let { open = false, onclose, onopendocs }: { open?: boolean; onclose: () => void; onopendocs: () => void } =
		$props();
...
<Sheet {open} {onclose} title="More">
	<div class="more-group">
		...
		{#each VIEWS as v (v.id)}
			<button class="more-row" class:active={ui.tab === v.id} onclick={() => pickView(v.id)}>
				...
				<svg class="more-chev" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m9 18 6-6-6-6" /></svg>
			</button>
		{/each}
	</div>
	...
	<div class="more-group">
		...
		<label class="more-row as-field">
			<span class="more-ico">
				<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect width="18" height="7" x="3" y="3" rx="1.5" /><rect width="9" height="7" x="3" y="14" rx="1.5" /><rect width="5" height="7" x="16" y="14" rx="1.5" /></svg>
			</span>
		...
		<button class="more-row" onclick={share}>
			<span class="more-ico">
				<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M4 12v8a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2v-8" /><path d="m16 6-4-4-4 4" /><path d="M12 2v13" /></svg>
			</span>
	...
	<div class="more-group">
		...
		<button class="more-row" onclick={() => { onclose(); onopendocs(); }}>
			<span class="more-ico">
